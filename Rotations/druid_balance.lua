function druid_balance(self)
	...
	local focus_dotting, focus_is, focus_mf
	focus_dotting = false
	if UnitExists("focus") then
		focus_dotting = true
		focus_is = jps.debuff_duration("focus","insect swarm")
		focus_mf = jps.debuff_duration("focus","moonfire")
		if focus_mf == 0 then focus_mf = jps.debuff_duration("focus","sunfire") end
	end

	if cd("Force of Nature") == 0 and jps.UseCDs then
	...
	elseif focus_dotting and focus_is < 1.5 then
		spell = "insect swarm"
		jps.Target = "focus"
	elseif focus_dotting and focus_mf < 1.5 then
		spell = "moonfire"
		jps.Target = "focus"
	...
	if focus_dotting and not ud("focus","earth and moon") and (spell == "wrath" or spell == "starfire") then
		jps.Target = "focus"
	end
